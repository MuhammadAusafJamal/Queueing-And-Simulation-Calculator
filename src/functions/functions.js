...
function createCustomer(customerData) {
  return { ...customerData, servedTime: 0 };
}
function allCustomersServed(customers) {
  return customers.every(
    (customer) => customer.servedTime >= customer.serviceTime
  );
}
function simulate(customers) {
  let currentTime = 0;
  ...
  while (!completed) {
    ...
    if (safetyCounter > 100) {
      console.log("Loop breaking due to excessive iterations.");
      break;
    }

    // Find the next customer based on priority
    for (let customer of customers) {
      if (
        customer.arrival <= currentTime &&
        customer.servedTime < customer.serviceTime &&
        (nextCustomer === null || customer.priority < nextCustomer.priority)
      ) {
        nextCustomer = customer;
      }
    }
    ...
    if (nextCustomer) {
      if (!("startTime" in nextCustomer)) {
        nextCustomer.startTime = currentTime;
      }

      nextCustomer.servedTime++;

      if (nextCustomer.servedTime >= nextCustomer.serviceTime) {
        nextCustomer.endTime = currentTime + 1; // Ensure `endTime` is set
      }

      currentTime++; // Move forward in time
    } else {
      currentTime++; // No customer served? Move time forward
    }

    completed = allCustomersServed(customers);
  }

  // 🔴 **Ensure `endTime` for any missing customers**
  for (let customer of customers) {
    if (!("endTime" in customer) && customer.servedTime > 0) {
...
function prioritySimulation(data) {
  if (!data) return [];
  let customers = data.map(createCustomer);
  return simulate(customers);
}
export const calculateCalculatedDataPriority = (data) => {
  ...
  for (const customer of result) {
    ...
    if (endTime === undefined) {
      console.error(`Customer ${customerId} is missing endTime!`);
  ...
  const totalTime = Math.max(...result.map((item) => item.endTime), 1);
  const averageWaitTime = totalWaitTime / (calculatedData.length || 1);
  const averageTurnaroundTime = totalTurnaroundTime / (calculatedData.length || 1);
  const serverUtilization = serverUtilizationTime / totalTime;
  const serverIdle = 1 - serverUtilization;
